...
test('onBack só renderiza quando passado', () => {
  const noop = () => {};
  const { rerender } = render(<ChatHeader title="Conversas" onMinimize={noop} onClose={noop} />);
  expect(screen.queryByRole('button', { name: /voltar/i })).not.toBeInTheDocument();

  const onBack = jest.fn();
  rerender(<ChatHeader title="Conversas" onMinimize={noop} onClose={noop} onBack={onBack} />);
