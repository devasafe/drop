...
const PAY_METHOD_LABEL: Record<string, string> = {
  ...
  money: 'Dinheiro', cash_on_delivery: 'Na entrega',
};

/**
 * GET /wallets/:userId/extract — extrato unificado do cliente para a carteira.
 *
 * Junta DUAS fontes (display-only, sem alterar contabilidade):
 *  - COMPRAS: dos Orders pagos do cliente (mostra loja + método + valor). Aparecem
 *    mesmo quando pagas por PIX/cartão (que não passam pela carteira).
 *  - MOVIMENTOS de carteira NÃO-compra (deposit/withdrawal/refund/transfer/penalty)
 *    dos WalletEntry. As entradas 'payment' (parte paga com saldo) são omitidas para
 *    não duplicar a compra que já vem do pedido.
 */
export const getClientExtract = async (req: Request, res: Response) => {
  try {
    const { userId } = req.params;
    if (String((req as any).user?.id) !== String(userId)) {
      return res.status(403).json({ error: 'Acesso negado' });
    ...
    const PAID_STATUSES: any[] = ['pago', 'aguardando_motoboy', 'enviado', 'entregue'];
    const [orders, entries] = await Promise.all([
      prisma.order.findMany({
        where: { customerId: String(userId), status: { in: PAID_STATUSES } },
        include: { store: { select: { name: true } } },
        orderBy: { createdAt: 'desc' },
        take: limit,
      }),
      prisma.walletEntry.findMany({
        where: { walletId: wallet.id, category: { not: 'payment' } },
        orderBy: { createdAt: 'desc' },
        take: limit,
      }),
    ]);
    ...
    const compras = orders.map((o: any) => ({
      ...
      title: `Pedido #${String(o.id).slice(-6).toUpperCase()}`,
      subtitle: `${o.store?.name || 'Loja'} · ${PAY_METHOD_LABEL[String(o.paymentMethod)] || 'Pagamento'}`,
      storeName: o.store?.name || 'Loja',
      paymentMethod: o.paymentMethod,
      status: o.status,
      relatedId: o.id,
    }));

    const movimentos = entries.map((e: any) => ({
      kind: e.category as string,
      date: e.createdAt,
      amount: Number(e.amount),
      type: e.type as string,
      category: e.category as string,
      title: e.reason || e.category,
      subtitle: e.paymentMethod ? (PAY_METHOD_LABEL[String(e.paymentMethod)] || undefined) : undefined,
      status: undefined,
      relatedId: e.reference || undefined,
    }));

    const extract = [...compras, ...movimentos]
      .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())
      .slice(0, limit);

    return res.json({ extract });
  } catch (err: any) {
    console.error('[CLIENT EXTRACT ERROR]', err);
